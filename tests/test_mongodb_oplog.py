...
class MongoDBOplog(unittest.TestCase):
    ...
    def test_run(self):
        ...
        changed_ids_deletes = set()
        with get_test_connection() as client:
            # Delete two documents for each collection

            changed_ids_deletes.add(client['simple_db']['simple_coll_1'].find({'int_field': 0})[0]['_id'])
            client["simple_db"]["simple_coll_1"].delete_one({'int_field': 0})

            changed_ids_deletes.add(client['simple_db']['simple_coll_1'].find({'int_field': 1})[0]['_id'])
            client["simple_db"]["simple_coll_1"].delete_one({'int_field': 1})

            changed_ids_deletes.add(client['simple_db']['simple_coll_2'].find({'int_field': 0})[0]['_id'])
            client["simple_db"]["simple_coll_2"].delete_one({'int_field': 0})

            changed_ids_deletes.add(client['simple_db']['simple_coll_2'].find({'int_field': 1})[0]['_id'])
            client["simple_db"]["simple_coll_2"].delete_one({'int_field': 1})

        # Run sync and verify the deletes
        sync_job_name = runner.run_sync_mode(self, conn_id)
        ...
        # Verify that we got at least 2 records due to deletes
        ...
        # gather the actual _ids from the replicated data
        ...
        # since there is an overlap in log based we need to validate if there is only 1 extra _id of the overlapping document in the actual
        self.assertGreaterEqual(len(actual), 4)

        # Verify that the changed_ids is a subset of the actual _ids
        self.assertTrue(changed_ids_deletes.issubset(actual))

        ################ Isolating Updates #################

        changed_ids_updates = set()
        with get_test_connection() as client:

            # Update two documents for each collection
            changed_ids_updates.add(client['simple_db']['simple_coll_1'].find({'int_field': 48})[0]['_id'])
            client["simple_db"]["simple_coll_1"].update_one({'int_field': 48},{'$set': {'int_field': -1}})

            changed_ids_updates.add(client['simple_db']['simple_coll_1'].find({'int_field': 49})[0]['_id'])
            client["simple_db"]["simple_coll_1"].update_one({'int_field': 49},{'$set': {'int_field': -1}})

            changed_ids_updates.add(client['simple_db']['simple_coll_2'].find({'int_field': 98})[0]['_id'])
            client["simple_db"]["simple_coll_2"].update_one({'int_field': 98},{'$set': {'int_field': -1}})

            changed_ids_updates.add(client['simple_db']['simple_coll_2'].find({'int_field': 99})[0]['_id'])
            client["simple_db"]["simple_coll_2"].update_one({'int_field': 99},{'$set': {'int_field': -1}})

        # Run sync and verify the updates
        sync_job_name = runner.run_sync_mode(self, conn_id)
        ...
        # Verify that we got at least 2 records due to updates
        # (could be more due to overlap in gte oplog clause)
        for k, v in record_count_by_stream.items():
            self.assertGreaterEqual(v, 2)
        ...
        # Verify that the changed_ids is a subset of the actual _ids
        self.assertTrue(changed_ids_updates.issubset(actual))

        ########### Isolating inserts #############

        changed_ids_inserts = set()
        with get_test_connection() as client:
            ...
            changed_ids_inserts.add(client['simple_db']['simple_coll_1'].find({'int_field': 50})[0]['_id'])

            client["simple_db"]["simple_coll_1"].insert_one({"int_field": 51, "string_field": random_string_generator()})
            changed_ids_inserts.add(client['simple_db']['simple_coll_1'].find({'int_field': 51})[0]['_id'])

            client["simple_db"]["simple_coll_2"].insert_one({"int_field": 100, "string_field": random_string_generator()})
            changed_ids_inserts.add(client['simple_db']['simple_coll_2'].find({'int_field': 100})[0]['_id'])

            client["simple_db"]["simple_coll_2"].insert_one({"int_field": 101, "string_field": random_string_generator()})
            changed_ids_inserts.add(client['simple_db']['simple_coll_2'].find({'int_field': 101})[0]['_id'])

        # Run sync and verify the inserts
        sync_job_name = runner.run_sync_mode(self, conn_id)
        ...
        # Verify that we got at least 2 records due to inserts
        # (could be more due to overlap in gte oplog clause)
        for k, v in record_count_by_stream.items():
            self.assertGreaterEqual(v, 2)
        ...
        # Verify that the changed_ids is a subset of the actual _ids
        self.assertTrue(changed_ids_inserts.issubset(actual))
