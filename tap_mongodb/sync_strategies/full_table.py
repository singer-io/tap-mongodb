...
def sync_collection(client, stream, state, projection):
    ...
    while retries < common.MAX_CURSOR_RETRIES:
        try:
            with collection.find({'_id': find_filter},
                ...
                # Cursor finished normally, exit the retry loop
                break

        except CursorNotFound as e:
            retries += 1
            if retries >= common.MAX_CURSOR_RETRIES:
                raise

            resume_id = singer.get_bookmark(state, stream['tap_stream_id'], 'last_id_fetched')
            last_id_fetched_type = singer.get_bookmark(state, stream['tap_stream_id'], 'last_id_fetched_type')
            find_filter['$gte'] = common.string_to_class(resume_id, last_id_fetched_type)
            LOGGER.warning('CursorNotFound for %s after %s rows (retry %s/%s). '
                           'Resuming from last checkpoint %s. Error: %s',
                           tap_stream_id, rows_saved, retries, common.MAX_CURSOR_RETRIES,
                           resume_id, str(e))
